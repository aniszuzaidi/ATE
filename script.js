...
function filterFoods(foods, prefs) {
  return foods.filter(food => {
    ...
    // --- Distance filter ---
    if (prefs.distance === 'nearby' && food.distance > 2) return false;
    if (prefs.distance === 'farther' && (food.distance <= 2 || food.distance > 5)) return false;
...
function showResult(food) {
  ...
  // Update "Find Near Me" button label (nearby.js)
  if (typeof updateNearbyButton === 'function') updateNearbyButton(food.name);
...
function hideResult() {
  ...
  // Also close the nearby section if open
  if (typeof closeNearby === 'function') closeNearby();
}
...
function resetFilters() {
  ...
  setGroupActive('distanceGroup', 'anywhere');

 ...
 *  3. A Google Maps embed iframe loads showing nearby restaurants for that food
 *  4. Buttons to open full Google Maps / Waze are shown below
 *
 * No API key required. Works on all platforms (Netlify, Render, etc.)
 */
...
// Fallback: manually search without GPS
window.startManualSearch = function() {
  ...
  // Show a placeholder map frame with a Google Maps search
...
function loadGoogleMap(lat, lng, foodName) {
  ...
  // Google Maps Embed API — no API billing needed for basic searches
  const embedUrl = 'https://www.google.com/maps/embed/v1/search'
    + '?key=AIzaSyD-9tSrke72PouQMnMX-a7eZSW0jkFMBWY'
    + '&q=' + encodeURIComponent(searchQuery)
    + '&center=' + lat + ',' + lng
  ...
  // Full Google Maps link (opens the real app)
  const fullUrl = 'https://www.google.com/maps/search/' + encodeURIComponent(searchQuery) + '/@' + lat + ',' + lng + ',15z';
  // Waze link
  const wazeUrl = 'https://waze.com/ul?ll=' + lat + ',' + lng + '&navigate=yes&q=' + encodeURIComponent(searchQuery);
